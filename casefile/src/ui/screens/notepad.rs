...
use super::footer;
use crate::ui::{App, BOLD, HILITE, Key, MUTED, Screen};
...
pub(crate) fn render(app: &App, area: Rect, buf: &mut Buffer) {
    let g = app.game();
    let p = &g.case.puzzle;
    let hint = if app.pad_notes {
        "type to edit  Enter newline  Tab grid  Esc back"
    } else {
        "hjkl move  Space cycle  x no  o yes  Backspace clear  Tab notes  Esc back"
    };
    ...
    let block = Block::bordered().title(" Notepad ");
    ...
    for (ci, e) in cols.iter().enumerate() {
        ...
        if e.item == 0 {
            put(buf, x, inner.y, &p.categories[e.cat].name, BOLD);
        }
        let item: String = p.name(*e).chars().take(text_w).collect();
        put(buf, x, inner.y + 1, &item, MUTED);
    }
    for (r, s) in g.case.suspects.iter().enumerate() {
        ...
        for (ci, e) in cols.iter().enumerate() {
            ...
            put(buf, x, y, sym, if hot { HILITE } else { BOLD });
        }
    }
    ...
    put(buf, inner.x, inner.y + 2 + m, &legend, MUTED);

    let title = if app.pad_notes {
        " Notes (editing) "
    } else {
        " Notes "
    };
    let block = Block::bordered().title(title);
    let inner = block.inner(notes_area);
    block.render(notes_area, buf);
